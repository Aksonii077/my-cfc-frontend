document.addEventListener('DOMContentLoaded', function() {
    ...
    async function tryGetJWTFromApp() {
        try {
                ...
                func: () => {
                    ...
                    if (token && userData) {
                        try {
                            ...
                            return { token: token, userInfo: { name: 'User', email: '' } };
                        }
                    }
                    return null;
                }
            ...
            if (results && results[0] && results[0].result) {
                ...
                showSuccessMessage('✅ Token synced from React app!');
            } else {
    ...
    async function startSyncing() {
        ...
        try {
            ...
            if (!contentScriptStatus.loaded) {
                ...
                // Test ping again
                const pingTest = await checkContentScriptLoaded();
                console.log('📋 Ping test after injection:', pingTest);
    ...
    async function checkExistingConnections(jwtToken) {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/linkedin/connections?all=true`, {
                method: 'GET',
                headers: {
