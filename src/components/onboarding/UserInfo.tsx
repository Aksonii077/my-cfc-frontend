...
const UserInfo = memo(({ onComplete }: UserInfoProps) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.error(
        "[USER_INFO] Error restoring data from localStorage:",
        error
      );
    }
  ...
  const validateField = useCallback(
    (field: keyof UserInfoData, value: string): string => {
      switch (field) {
        case "firstName":
        case "lastName":
          return value.trim()
  ...
  const updateField = useCallback(
    (field: keyof UserInfoData, value: string) => {
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-2xl shadow-lg">
        ...
        <CardContent className="space-y-4">
          ...
          />
          <SubmitSection isSubmitting={isSubmitting} onSubmit={handleSubmit} />
          <PrivacyNotice />
        </CardContent>
      </Card>
    </div>
...
const RoleSelectionSection = memo(
  ({
    ...
    const clickableRoles = ["founder", "mentor", "service_provider"];
